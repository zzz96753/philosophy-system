...
% 标题用色，但每个标题后把正文重置为纯黑（防止标题色漏入正文）
\usepackage{etoolbox}
\makeatletter
\apptocmd{\@afterheading}{\color{black}}{}{}
\makeatother
...
陈述式的总览图画的是“谁属于哪一类”。深度思考需要另一张图：\keyq{谁反驳谁、谁回应谁}——把全文骨架收成一张图：\textbf{一个问题 → 四种视角发难 → 汇成一个立场}。

\begin{center}
\includegraphics[width=0.86\linewidth]{img/tension-map.pdf}\\[0.6em]
{\notolight\footnotesize\color{notebar} 灰色箭头 = 各视角的发难线　·　红色箭头 = 汇入“我的综合立场”}
\end{center}
